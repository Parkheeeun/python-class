...
# - 괄호 안 영문자를 입력하면(대소문자 구분안함) 각 기능이 구현되게 만든다
...
# - 출생년도(birthyear) : 4자리로 입력,숫자로 입력되어야함
...
while True:
    ...
    if choice=="I":        
        ...
        inputgender=input('성별을 입력하세요(M OR F)>>')
        inputemail=input('이메일을 입력하세요>>')
        inputbirthyear=input('출생년도를 입력하세요>>')
        
        while True:
            inputgender=inputgender.upper()
            if inputgender == 'M' or inputgender=='F':
                False
                break
            else:
                inputgender=input("성별을 다시 입력하세요>>")
                True

        while True:
            p=re.compile('[a-z][a-z0-9]{4,}@[a-z]{2,}[.](kr|com|org|net)$')
            result=p.match(inputemail)
            if result==None:
                inputemail=input("이메일을다시입력하세요>>")
                True
            else:
                break
        while True:
            if inputbirthyear.isdigit()==False or len(inputbirthyear)!=4:
                inputbirthyear=input('출생년도를 다시입력해주세요>>')
                True
            else:
                break
        customer={'name':inputname, 'gener':inputgender,'email':inputemail, 'birthyear':inputbirthyear}
        print(customer)
        ...
        if page<0:
            ...
            print(f'현재페이지는 {page+1}페이지입니다')
        print(custlist)
    elif choice == 'P':
        if page<=0:
            print('첫번째페이지입니다. 이전페이지로 이동불가')
        else:
            page -=1
            print(f'현재페이지는{page+1}페이지입니다')
            print(custlist[page])
        ...
        print(custlist)
        inputemail=input('삭제하려는email>>')
        delok=0
        for i in range(len(custlist)):
            if inputemail == custlist[i]['email']:
        ...
        while True:
            ...
            if idx ==-1:
                ...
                break
            choice1=input('''
            다음중 수정할 항목을 선택하세요(name,gender,birth)
            >>> ''')
            custlist[idx][choice1]=input('수정할{}을입력하세요>>'.format(choice1))
            print(custlist)
            break
    elif choice=="Q":
        break
